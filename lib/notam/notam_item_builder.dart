...
class NotamItemBuilder {
  var jsonNotamList = <NotamJson>[];
  bool? sortCategories;
  var result = NotamModelList();

  NotamItemBuilder({required this.jsonNotamList, this.sortCategories}) {
    ...
    for (var i = 0; i < jsonNotamList.length; i++) {
      ...
      if (jsonNotamList[i].airportNotFound!) {
        ...
        List<AirportNotam>? finalNotamItemList = <AirportNotam>[];
        if (sortCategories!) {
          ...
          var sortedNotamList = <AirportNotam>[];
